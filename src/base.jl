...
function rhs(dx, x, sys::ODESys, t=0.0) ::Nothing end
function jac(J, x, sys::ODESys, t=0.0) ::Nothing end
...
Base.broadcastable(sys::ODESys) = Ref(sys)

# generic out-of-place versions of rhs & jac, for convience
function rhs(x, sys::ODESys, t = 0.0)
    dxdt = similar(x)
    rhs(dxdt, x, sys, t)
    return dxdt
end

function jac(x, sys::ODESys, t = 0.0)
    n = length(x)
    J = similar(x, n, n)
    jac(J, x, sys, t)
    return J
